...
class dataframe(Dataset):
    def __init__(self,x,y,tokenizer,max_len,transform=None):
        ...
        self.x=x  #assign the data set to a variables so that we can process it in the function
        self.y=y.to_numpy()
...
dataloader_test=DataLoader(dataset_test,batch_size=batch_size_test,shuffle=True,num_workers = 2)

#Define the model used, in this case, "BertForSequenceClassification" is used
...
for i in range(epochs):
    ...
    for batch in dataloader_train:
        ...
        optimizer.zero_grad()
        Final_input={'input_ids':input_ids,'attention_mask': attention_mask,'labels':target}
        outputs=model(**Final_input)
        predicted_prob = outputs.logits
        predicted_label = predicted_prob.argmax(dim=1)    ##return the max tensor position in each tensor array
        loss = outputs.loss
        correct_predictions += ((predicted_label == target.view(-1)).sum()).item()   ## check the predicted= given label and sum up the correct preedictions, the view() is used to reshape the tensor array
        loss.backward()                                          
        j+=1
